...
const answerSpan = document.getElementById('answer') as HTMLSpanElement | null;

async function fetchAndUpdateAnswer(): Promise<void> {
    if (!answerSpan) {
    ...
    try {
        const response = await fetch(BACKEND_API_GET_URL);
        
        if (!response.ok) {
            throw new Error(`HTTP error! Status: ${response.status} - ${response.statusText}`);
        }

        const result: { data: string | null } = await response.json();
